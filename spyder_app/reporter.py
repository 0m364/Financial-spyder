...
class Reporter:
    ...
    def _sanitize_for_csv(self, value):
        ...
        if isinstance(value, str) and value:
            ...
            if value and (
                value[0] in dangerous_chars or value.lstrip()[0:1] in dangerous_chars
            ):
                return "'" + value
        return value
    ...
    def generate_pdf(self, filename, tier="Free"):
        ...
        pdf.cell(
            200,
            10,
            txt=f"Market Report ({tier} Version): {self.ticker}",
            ln=True,
            align="C",
        )
        pdf.ln(10)
        ...
        if self.technicals:
            pdf.cell(
                200,
                8,
                txt=f"Price: ${self.technicals.get('Current_Price', 0):.2f}",
                ln=True,
            )
            pdf.cell(200, 8, txt=f"RSI: {self.technicals.get('RSI', 0):.2f}", ln=True)
            pdf.cell(
                200, 8, txt=f"SMA 50: ${self.technicals.get('SMA_50', 0):.2f}", ln=True
            )
            pdf.cell(
                200,
                8,
                txt=f"SMA 200: ${self.technicals.get('SMA_200', 0):.2f}",
                ln=True,
            )

            if tier == "Premium":
                pdf.cell(
                    200, 8, txt=f"MACD: {self.technicals.get('MACD', 0):.4f}", ln=True
                )
                pdf.cell(
                    200,
                    8,
                    txt=f"Stochastic K: {self.technicals.get('Stoch_K', 0):.2f}",
                    ln=True,
                )
                pdf.cell(
                    200, 8, txt=f"ATR: {self.technicals.get('ATR', 0):.4f}", ln=True
                )
    ...
    def _add_weather_forecast(self, pdf):
        ...
        condition = "Cloudy"
        if score > 70:
            condition = "Sunny (Bullish)"
        elif score > 60:
            condition = "Partly Cloudy (Slightly Bullish)"
        elif score < 30:
            condition = "Stormy (Bearish)"
        elif score < 40:
            condition = "Rainy (Slightly Bearish)"
